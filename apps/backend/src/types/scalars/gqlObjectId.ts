...
export const GraphQLObjectId = new GraphQLScalarType({
  ...
  serialize(value: any): string {
    ...
    if (!isValidMongoDBObjectID(result)) {
      throw new GraphQLError(
        'serialize: value: ' + result + ' is not valid ObjectId',
      );
    }
  ...
  parseValue(value): Types.ObjectId {
    if (!isValidMongoDBObjectID(value as string)) {
      throw new GraphQLError(
        'parseValue: not a valid ObjectId string, require a string with 12 or 24 hex chars, found: ' +
          value,
      );
    }
